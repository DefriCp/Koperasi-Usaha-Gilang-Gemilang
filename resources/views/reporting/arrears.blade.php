<x-app-layout>
  ...
  <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-5 mb-5">
      <form method="GET" action="{{ route('reporting.arrears') }}" class="grid gap-3 md:grid-cols-12">
        <div class="md:col-span-4">
          ...
          <select name="month" class="h-11 w-full rounded-lg border border-gray-300 px-3">
            ...
            @foreach($monthNames as $i=>$label)
              <option value="{{ $i }}" @selected(isset($filters['month']) && (int)$filters['month']===$i)>{{ $label }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="md:col-span-2 flex items-end gap-2">
          ...
          <a
            href="{{ route('reporting.arrears.export', [
          ...
          <a href="{{ route('reporting.outstanding', ['year'=>$filters['year'] ?? now()->year]) }}"
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          ...
          <tr class="text-gray-900">
            ...
            <th class="px-4 py-3 text-right font-semibold">Tunggakan</th>
          </tr>
          </thead>
          ...
          @forelse($rows as $r)
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-right text-red-600 font-semibold">Rp {{ number_format($r->arrears,0,',','.') }}</td>
            </tr>
          @empty
            <tr>
              <td colspan="13" class="px-4 py-10 text-center text-gray-600 font-medium">
                Tidak ada data menunggak untuk filter yang dipilih.
              </td>
            </tr>
          @endforelse
          </tbody>
